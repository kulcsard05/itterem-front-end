...
- the seed generator currently works only on macOS because both placeholder and Pexels image processing shell out to the built-in `sips` tool to produce JPEG blobs
...
- the script uses `sips` to normalize images into JPEG blobs, so it is currently macOS-dependent
